...
import type { PropertyMode } from '@/types'

export function Propiedades() {
  const [mode, setMode] = useState<PropertyMode>('venta')
      ...
      <div className="max-w-[1300px] mx-auto">
        ...
        <ScrollReveal delay={0.05}>
          <div className="flex gap-2 mb-10">
            {(['venta', 'alquiler'] as PropertyMode[]).map((m) => (
              <button
                key={m}
                onClick={() => setMode(m)}
          ...
          <AnimatePresence mode="popLayout">
            {filtered.map((prop, i) => (
              ...
              >
                ...
                <div className="p-5">
                  <p className="font-display text-[22px] font-medium text-charcoal mb-1.5">
                    {prop.price}
                  </p>
                  ...
                  <div className="flex gap-4 pt-3.5 border-t border-cream-dark text-[10px] font-medium text-warm">
                    <span className="flex items-center gap-1.5">
                      <Maximize2 size={12} /> {prop.area}
                    </span>
                    <span className="flex items-center gap-1.5">
                      <Bed size={12} /> {prop.bedrooms} hab.
                    </span>
                    <span className="flex items-center gap-1.5">
                      <Bath size={12} /> {prop.bathrooms} baños
                    </span>
                  </div>
                </div>
              </motion.article>
